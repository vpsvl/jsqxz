...
function NGTX_sm(ng,tx)
    ...
    ------------------------【受击类】特效----------------------------	
        ...
        if tx == 801 then 
            --local hh =  kfcj
            local gl = ((kfcj+1)*15)..'%'
            txwz = {'混沌太玄','Ｗ受到伤害时将杀气的'..cl2..gl..'Ｗ转化为自身的集气。将所受伤害的'..cl2..gl..'Ｗ转化为生命回复。【需挑战解锁】',4}
        end
        if tx == 802 then 
            --local hh =  kfcj
            local gl = (kfcj*6)..'%'
        ...
        if tx == 824 then 
            local gl = (kfcj*5)..'%'
            txwz = {'闪避','Ｗ受到攻击时'..cl2..gl..'Ｗ无视必中，独立概率闪避攻击。',4}
        end
        if tx == 825 then 
            local gl = (kfcj*10)..'%'
    ...
    ------------------------【攻击类】特效----------------------------	
        ...
        if tx == 933 then 
            local hh =  kfcj*500
            local gl = (kfcj*25)..'%'
            txwz = {'龙象之力','Ｗ攻击时'..cl2..gl..cl..'概率随自身怒气值增加伤害,最多增加'..cl2..(kfcj*25)..cl..'%,最多增加气攻'..cl2..hh..cl..'点。',4}
        end
        if tx == 934 then 
            --local hh =  kfcj*250
            local gl = (kfcj*20)..'%'
    ...
    ------------------------【特殊效果】------------------------------	
        ...
        if tx == 1022 then 
            local hh =  5 - kfcj 
            local gl = kfcj*25
            txwz = {'佛光普渡','Ｗ每'..cl2..hh..'回合'..gl..'%'..cl..'概率清除流血、中毒、冰封、燃烧、内伤状态。',4}
        end
        if tx == 1023 then 
            local hh =  5 - kfcj 
            local gl = kfcj*25
            txwz = {'抱元守一','Ｗ每'..cl2..hh..'回合'..gl..'%'..cl..'概率清除大部分异常状态。',4}
        end
        if tx == 1024 then 
            --local hh =  kfcj*2 
            local gl = kfcj+1
        ...
        if tx == 1039 then 
            local gl = (kfcj*15)..'%'
            local gl1 = (kfcj*5)..'%'      
            txwz = {'太上忘情','Ｗ攻击时'..cl2..gl..cl..'触发风流/火延/云翳之一，风流：'..cl2..gl..cl..gl1..'概率魅惑同性敌方，'
                    ..cl2..gl..cl..'概率魅惑异性敌方；火延：给全体敌方追加灼烧值;云翳：攻击后10时序免疫伤害，冷却40时序。受到攻击时'..gl..'概率触发君王/我无/师教之一，：君王：敌全体恐惧一回合，师教：敌全体集气-200，我无：免疫伤害一次。',4}
        end
        if tx == 1040 then 
            local hh = kfcj*10
            local gl = (kfcj*50)..'%'
        ...
        if tx == 1041 then 
            local hh = kfcj*10
            local gl = (kfcj*50)..'%'
            txwz = {'暂无','Ｗ暴怒时集气速度增加'..cl2..gl..cl..'，且集气不受异常状态影响。',4}
        end
        if tx == 1042 then 
            local hh = kfcj*10
            local gl = (kfcj*50)..'%'
        ...
        if tx == 1043 then 
            local hh = kfcj*10
            local gl = (kfcj*50)..'%'
            txwz = {'回合回内','Ｗ每回合回复身内力'..cl2..gl..cl..'点',2}
        end
