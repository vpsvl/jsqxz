...
export function yunZhuanQianKun(lv) {
  return {
    ...
    name: `运转乾坤 Lv${lv}`,
    effect: [
      '自身九格内的我方队友受到攻击时，转移攻击者的攻击落点到敌方友军处，新落点不会覆盖原范围内的我方友军，新范围内目标受全额伤害（无误伤）',
      `（战胜摩尼后可激活）`,
    ],
  };
}
...
export function jiuYinGuiYi(lv) {
  return {
    ...
    effect: [
      '免疫debuff',
      `攻击时${lv * 15}%概率给敌方增加3~5层【迟缓状态】，【迟缓状态】达到十层会触发【玄阴入体】：集气归0，天赋，奇穴，秘技，内功效果都不触发，需要10时序内消耗最大内力值10%（无内力则消耗生命值）来解除`,
      `（射雕正线王重阳送九阴上部可激活）`,
    ],
  };
}
