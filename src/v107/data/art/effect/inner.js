...
export function hunDunTaiXuan(lv) {
  return {
    ...
    effect: [
      `受到伤害时触发：`,
      `└五岳倒为轻：${(lv + 1) * 6}%概率触发，将杀气的${lv * 15}%转化为自身的集气；下一次受到伤害时，将所受伤害的${lv * 30}%转化为生命恢复（因为是延迟到下一次受到伤害才会恢复生命，所以生命值不够的话是可以被直接击杀的）`,
      `└救赵挥金锤：不触发【五岳倒为轻】${(lv + 1) * 6}%概率打断敌方攻击，并使敌方全体集气位置减${lv * 50}点`,
    ],
  };
}
...
export function fanTanZengQiang(lv) {
  return {
    id: 905,
    name: `反弹·增强 Lv${lv}`,
    effect: [`被攻击时${lv * 25}%概率反弹的目标增加1个，反弹伤害增加10%，如果只有一个目标，则受到双倍反弹伤害`],
  };
}
...
export function longXiangZhiLi(lv) {
  return {
    ...
    name: `龙象之力 Lv${lv}`,
    effect: [
      `攻击时${lv * 25}%概率随自身怒气值增加伤害，最多增加${lv * 15}%，最多增加气攻${lv * 500}点`,
    ],
  };
}
...
export function chongSheng(lv) {
  return {
    id: 1005,
    name: `重生 Lv${lv}`,
    effect: [`被击败时${lv * 25}%概率重生，恢复${lv * 25}已损失生命、内力和体力，清除基础状态，并且打断敌方的攻击立即行动`],
  };
}
...
export function buXi(lv) {
  return {
    id: 1014,
    name: `不息 Lv${lv}`,
    effect: [`杀气被杀到-500，清除全部内伤，并恢复15%生命，每回合最多触发3次`],
  };
}
...
export function sanYuanGuiYi(lv) {
  return {
    ...
    effect: [
      ...
      `天霜劲+虚云劲+神风劲之和大于10层，则消耗所有三元状态且清空虚云劲增加的气防，恢复自身状态50%，并且当前回合免疫伤害`,
    ],
  };
}
...
export function xuanZhiTaiYi(lv) {
  return {
    ...
    name: `玄之太一 Lv${lv}`,
    effect: [
      `攻击或受到攻击时30%概率触发【玄之太一】，让敌方的奇穴不生效`,
      `（侠客行正线侠客岛战胜李白，邪线侠客岛战胜石破天可激活）`,
    ],
  };
}
...
export function guangMingJingShi(lv) {
  return {
    ...
    name: `光明净世 Lv${lv}`,
    effect: [
      '免疫灼烧和点燃外的负面状态，并使免疫灼烧和点燃无效，不受灼烧和点燃的负面效果',
      '当灼烧达到100时，如果受到致命伤害触发【生死劫】，免疫此次攻击（CD99时序）',
      '当受到负面效果效果时，触发【极乐引】概率将负面状态变成灼烧或点燃',
      `（倚天正线拿到天书后挑战光明圣火阵可激活）`,
    ],
  };
}

export function yunZhuanQianKun(lv) {
  return {
    id: 1055,
    name: `运转乾坤 Lv${lv}`,
    effect: [
...
export function jiuYangGuiYi(lv) {
  return {
    id: 1056,
    name: `九阳归一 Lv${lv}`,
    effect: [
      '免疫控制，受到阳性武功的攻击额外减少伤害10%',
      '每99时序为自身增加一个【紫气东来】状态，此状态下，可消耗一层【紫气东来】免疫一次致死伤害',
      `（战胜斗酒僧后可激活）`,
    ],
  };
}

export function jiuYinGuiYi(lv) {
  return {
    id: 1057,
    name: `九阴归一 Lv${lv}`,
    effect: [
...
export function youDingSiGu(lv) {
  return {
    ...
    name: `幽鼎饲蛊 Lv${lv}`,
    effect: [
      '以蛊虫饲养自身，根据受到的基础负面触发不同效果：',
      '└中毒时激发【枯木蛊】：每点中毒增加自身的攻防轻各1点',
      '└冰封时激发【玄冰蛊】：受到的伤害大于生命最大值20%时冻结自己5时序，且冻结期间免疫伤害；如果受到伤害时自身已经处于冻结状态则不会触发【玄冰蛊】',
      '└灼烧时激发【火蝶蛊】：点燃五格内的所有敌方5时序（CD30时序）',
      '└流血时激发【血魔母蛊】：在攻击者身上种下【血魔子蛊】10时序，拥有子蛊的敌人一起分担母蛊所受伤害',
    ],
  };
}
...
export function wanGuYinZhang(lv) {
  return {
    ...
    name: `万蛊引瘴 Lv${lv}`,
    effect: [
      '受到致命伤害时如果自身有激发蛊毒，则触发【万蛊引瘴】：免疫此次伤害，恢复生命最大值50%的生命值，并激活体内的【命蛊】，【命蛊】激活期间每回合结束时扣除生命最大值10%的生命值（保留1血不会致死），【命蛊】一旦被激活就会持续到战斗结束',
      '【万蛊引瘴】每回合最多触发一次，每场战斗最多触发三次',
    ],
  };
}
